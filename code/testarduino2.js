...
board.on("ready", () => {
  ...
  // Gestion des événements du bouton avec debounce
  let lastPressTime = 0;
  const DEBOUNCE_TIME = 200; // 200 ms

  button.on("press", () => {
    const now = Date.now();
    if (now - lastPressTime > DEBOUNCE_TIME && isCodeBeingEntered) {
      // Vérifier si la saisie du code est en cours
      lastPressTime = now;
  ...
  function triggerAlarm() {
    ...
    //buzzer.pulse(750); // Joue le son d'alarme pour 500ms
  }
  ...
  function startCodeEntry() {
    ...
    codeEntered = 0; // Réinitialiser le code entré au début de la saisie
    ...
    const timer = setTimeout(() => {
      if (codeEntered === 3) {
        ...
        ledRed.stop().off();
        ledYellow.stop().off();
        mqttClient.publish("alarm", "code_correct"); // Publier sur MQTT
  ...
  function resetState() {
    isLightLow = false;
    codeEntered = 0; // Réinitialiser ici
